...
describe('request metrics', () => {
  test('tracks aggregate and per-route request latency/error metrics', () => {
    resetApiMetrics();
    recordApiRequest('GET /api/status', 'GET', 200, 12);
    recordApiRequest('GET /api/status', 'GET', 200, 18);
    recordApiRequest('POST /api/start', 'POST', 503, 25);
  ...
  test('renders prometheus metrics payload', () => {
    resetApiMetrics();
    recordApiRequest('GET /api/status', 'GET', 200, 10);
