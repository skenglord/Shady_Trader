...
- **Test Coverage**: ❌ `npm run quality:coverage` currently fails the gate: lines=44.14% (min 50%), branches=73.24% (min 65%). Phase 1 coverage-test expansion was added on June 15, but `npm run test:coverage` could not be rerun in this shell because `npm` is unavailable. A targeted run exposed a `ZeroCopyBuffer.read()` live-view bug, a deterministic treatment-group signal id mismatch, and a BacktestService trade-shape mismatch; all were fixed. Full rerun still requires a Node-enabled shell. See `documentation/production_readiness_todo.md` for the production-readiness remediation plan.
...
- **Revised Audit Fix Plan (June 16, 2026)**: Created `documentation/revised_audit_fix_plan.md` after reviewing the original audit-fix proposal. The revised plan rejects the original as safe-to-run wholesale, marks stale items such as the validateWorker import and Monte Carlo Router test fix as already resolved, and separates low-risk correctness fixes from deferred structural refactors.
...
- [x] **REVISED AUDIT FIX PLAN CREATED (June 16, 2026)**: Added `documentation/revised_audit_fix_plan.md` with a safer phased remediation plan. The plan keeps low-risk correctness fixes (duplicate signal recording, `shadow_trades` schema cleanup with migration caveat, `TRADER_TOKEN_PLACEHOLDER` TDZ, structured request logging, `.gitignore` artifacts), defers stale or already-fixed items, and separates higher-risk refactors from audit remediation.
